# Evolution Battle Game - Documentation Index

This index provides a guide to all documentation in the repository.

## Core Documentation (Active)

These documents are the primary references for the game's systems:

### Game Systems
- [README.md](../README.md) - Project overview, quick start, and features
- [BATTLE_SYSTEM_DOCUMENTATION.md](../BATTLE_SYSTEM_DOCUMENTATION.md) - Complete battle system guide
- [LIVING_WORLD_DOCUMENTATION.md](../LIVING_WORLD_DOCUMENTATION.md) - Creature histories, skills, personalities, relationships
- [ECOSYSTEM_DOCUMENTATION.md](../ECOSYSTEM_DOCUMENTATION.md) - Hunger, foraging, and survival mechanics
- [ENVIRONMENTAL_SIMULATION_DOCUMENTATION.md](../ENVIRONMENTAL_SIMULATION_DOCUMENTATION.md) - **NEW!** Weather, terrain, day/night, and hazards
- [GENETICS_SYSTEM_DOCUMENTATION.md](../GENETICS_SYSTEM_DOCUMENTATION.md) - Genetics and trait inheritance
- [LINEAGE_SYSTEM_DOCUMENTATION.md](../LINEAGE_SYSTEM_DOCUMENTATION.md) - Genetic strains and evolution
- [PELLET_SYSTEM_DOCUMENTATION.md](../PELLET_SYSTEM_DOCUMENTATION.md) - Food resource system

### Technical Documentation
- [MODELS_DOCUMENTATION.md](../MODELS_DOCUMENTATION.md) - Core models (Creature, stats, abilities)
- [RENDERING_DOCUMENTATION.md](../RENDERING_DOCUMENTATION.md) - Visual rendering and animation systems
- [BATTLE_STORY_MODE_DOCUMENTATION.md](../BATTLE_STORY_MODE_DOCUMENTATION.md) - AI-powered battle narratives

### Special Features
- [DEEP_SIMULATION_DOCUMENTATION.md](../DEEP_SIMULATION_DOCUMENTATION.md) - Dwarf Fortress-style simulation features
- [RANDOM_TRAIT_SYSTEM_DOCUMENTATION.md](../RANDOM_TRAIT_SYSTEM_DOCUMENTATION.md) - Random trait generation
- [LETHAL_COMBAT_TRAITS_DOCUMENTATION.md](../LETHAL_COMBAT_TRAITS_DOCUMENTATION.md) - High-risk combat traits
- [GRASS_GROWTH_SYSTEM.md](../GRASS_GROWTH_SYSTEM.md) - Dynamic pellet growth system

### Development
- [CONTRIBUTING.md](../CONTRIBUTING.md) - Contribution guidelines
- [AGENT_COORDINATION.md](../AGENT_COORDINATION.md) - Team standards and integration goals
- [INTEGRATION_CHECKLIST.md](../INTEGRATION_CHECKLIST.md) - Implementation progress tracking
- [TEST_STATUS.md](../TEST_STATUS.md) - Test suite status

## Archived Documentation

Historical implementation summaries, tuning guides, and architecture documents have been moved to [archive/](archive/) for reference. These documents capture the evolution of the codebase but are superseded by the active documentation above.

### Categories in Archive
- Combat system implementations and improvements
- Genetics/breeding system refactorings
- Performance optimization summaries
- UI/UX overhaul documents
- Implementation summaries for major features

See [archive/README.md](archive/README.md) for a complete list.

## Quick Navigation

- **New to the project?** Start with [README.md](../README.md)
- **Working on battles?** See [BATTLE_SYSTEM_DOCUMENTATION.md](../BATTLE_SYSTEM_DOCUMENTATION.md)
- **Working on creatures?** See [LIVING_WORLD_DOCUMENTATION.md](../LIVING_WORLD_DOCUMENTATION.md)
- **Working on genetics?** See [GENETICS_SYSTEM_DOCUMENTATION.md](../GENETICS_SYSTEM_DOCUMENTATION.md)
- **Working on rendering?** See [RENDERING_DOCUMENTATION.md](../RENDERING_DOCUMENTATION.md)
